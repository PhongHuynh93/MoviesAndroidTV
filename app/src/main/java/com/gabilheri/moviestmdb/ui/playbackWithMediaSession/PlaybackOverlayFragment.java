 ...
 * info - In the source code, I implemented “UI update part” in PlaybackOverlayFragment.java,
 * while “Video control part” is implemented in PlaybackOverlayActivity.java.
 */
...
//        int currentItemIndex = (int) mSelectedMovie.getId() % 100;
    ...
    // step - in onattach and ondetach, we have to register the callback
    @Override
    ...
    // step - mean when start the app, start playing the video
    @Override
    public void onStart() {
    ...
    private void setUpRows() {
        ...
        /**
         * To control MediaSession from PlaybackControlsRow in VideoDetailsFragment, we use MediaController.
         * step - To assign action of each video control button, we use setOnActionClickedListener which is a method of PlaybackControlsRowPresenter.
         */
    ...
    // step - when we click something in the detail screen, need to notify
...
//            mPlaybackControlsRowTarget = new PicassoPlaybackControlsRowTarget(mPlaybackControlsRow);
            updateVideoImage(mItems.get(index).getCardImageUrl());
        }
    }

    // update the movvie content
...
//            ((BaseTvActivity) getActivity()).goToDetailMovie(itemViewHolder, item);
            // step - when enter the video that are playing - see the movie detail
...
//            }
    ...
    // step - We need to change the UI when video control action has executed, and video playback status has changed.
    private class MediaControllerCallback extends MediaController.Callback {
        ...
        // step - When MediaMetadata has changed
        // We can update item value of PlaybackControlsRow.
        @Override
